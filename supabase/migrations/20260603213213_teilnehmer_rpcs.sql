-- Teilnehmer-Zugriff über SECURITY-DEFINER-RPCs.
-- Teilnehmer authentifizieren sich anonym (auth.uid()); teilnehmer.auth_user_id verknüpft.
-- antwortoption.ist_richtig wird NIE an Teilnehmer ausgeliefert; Punkte serverseitig.

-- Beitreten / Reconnect: legt teilnehmer an oder gibt bestehenden zurück.
...
begin
  ...
  select * into v_tn from public.teilnehmer
    where pruefung_id = v_pr.id and auth_user_id = v_uid;
  ...
  if not found then
    ...
    insert into public.teilnehmer (pruefung_id, auth_user_id, name)
      values (v_pr.id, v_uid, trim(p_name))
      returning * into v_tn;
  end if;

  return jsonb_build_object(
    'teilnehmer_id', v_tn.id,
    'name', v_tn.name,
    'status', v_pr.status,
...
-- Status/Meta per Code (für Lobby-Polling, kein Fragen-Inhalt).
...
begin
  ...
  return jsonb_build_object(
    'status', v_pr.status,
    'start_zeit', v_pr.start_zeit,
    'end_zeit', v_pr.end_zeit,
    'late_join_modus', v_pr.late_join_modus,
    'dauer_minuten', v_dauer,
    'vorlage_name', v_name,
    'kurs_name', v_kurs
  );
end; $$;

-- Fragen (ohne Lösungs-Flag) + eigene bisherige Antworten. Markiert gestartet_am.
...
begin
  ...
  if v_pr.status not in ('laeuft', 'beendet') then raise exception 'Die Prüfung läuft nicht.'; end if;

  if v_tn.gestartet_am is null and v_pr.status = 'laeuft' then
  ...
  select jsonb_agg(jsonb_build_object(
      ...
      'optionen', (
        select jsonb_agg(jsonb_build_object('id', ao.id, 'text', ao.text) order by ao.sortierung)
  ...
  select jsonb_agg(jsonb_build_object(
      'pruefung_frage_id', a.pruefung_frage_id,
      'optionen', a.ausgewaehlte_optionen,
      'unsicher', a.unsicher_markiert))
  into v_antworten
  from public.antwort a where a.teilnehmer_id = v_tn.id;

  select dauer_minuten into v_dauer from public.pruefungsvorlage where id = v_pr.vorlage_id;
  v_personal_end := case
    when v_pr.late_join_modus = 'volle_zeit' and v_tn.gestartet_am is not null
      then v_tn.gestartet_am + make_interval(mins => v_dauer)
    else v_pr.end_zeit end;

  return jsonb_build_object(
    'status', v_pr.status,
    'end_zeit', v_personal_end,
    'abgegeben', v_tn.abgegeben_am is not null,
    'fragen', coalesce(v_fragen, '[]'::jsonb),
    'antworten', coalesce(v_antworten, '[]'::jsonb)
  );
end; $$;

-- Antwort speichern (Auto-Save). Nur während laeuft und vor Abgabe.
...
declare
  v_uid uuid := auth.uid();
  v_tn public.teilnehmer%rowtype;
  v_status text; v_pf_pruefung uuid;
begin
  select * into v_tn from public.teilnehmer where id = p_teilnehmer_id and auth_user_id = v_uid;
  if not found then raise exception 'Kein Zugriff.'; end if;
  if v_tn.abgegeben_am is not null then raise exception 'Bereits abgegeben.'; end if;
  select status into v_status from public.pruefung where id = v_tn.pruefung_id;
  if v_status <> 'laeuft' then raise exception 'Die Prüfung läuft nicht.'; end if;
  ...
  if v_tn.gestartet_am is null then
    update public.teilnehmer set gestartet_am = now() where id = v_tn.id;
  end if;
  ...
  on conflict (teilnehmer_id, pruefung_frage_id) do update
    set ausgewaehlte_optionen = excluded.ausgewaehlte_optionen,
        unsicher_markiert = excluded.unsicher_markiert,
        zuletzt_geaendert = now();
end; $$;

-- Abgeben + serverseitige Punkteberechnung (IHK-Schema). Idempotent.
...
begin
  ...
  select status into v_status from public.pruefung where id = v_tn.pruefung_id;
  if v_status not in ('laeuft', 'beendet') then raise exception 'Abgabe nicht möglich.'; end if;
  ...
  loop
    select ausgewaehlte_optionen into v_selected
      from public.antwort where teilnehmer_id = v_tn.id and pruefung_frage_id = rec.pf_id;
    if v_selected is null then v_selected := '{}'; end if;
    v_n := coalesce(array_length(v_selected, 1), 0);
    select count(*) into v_correct
      from public.antwortoption where frage_id = rec.frage_id and ist_richtig and id = any(v_selected);
  ...
  update public.teilnehmer
    set abgegeben_am = now(), punkte_gesamt = v_total, punkte_max = v_max, prozent = v_prozent
    where id = v_tn.id;

  return jsonb_build_object('punkte_gesamt', v_total, 'punkte_max', v_max, 'prozent', v_prozent);
end; $$;

-- Diese RPCs sind die Teilnehmer-API (interne Autorisierung via auth.uid()).
grant execute on function public.pruefung_beitreten(text, text) to anon, authenticated;
grant execute on function public.pruefung_status(text) to anon, authenticated;
grant execute on function public.pruefung_fragen(uuid) to anon, authenticated;
grant execute on function public.antwort_speichern(uuid, uuid, uuid[], boolean) to anon, authenticated;
grant execute on function public.pruefung_abgeben(uuid) to anon, authenticated;
